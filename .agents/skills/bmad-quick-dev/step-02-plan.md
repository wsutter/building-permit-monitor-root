...
1. Verify the current branch is `develop` (or `main` if no `develop` exists).
   - If not, halt and ask the user for confirmation to proceed.
2. Create/check out a feature branch:
   - Name: `feature/{story-key}` (e.g., `feature/1-1-csv-mapping`).
   - If the branch exists, check it out.
   - If creation fails, halt and ask for manual intervention.

### TDD: Test Planning
1. **Define Test Cases**
   - Write test cases for the feature (unit, integration, or end-to-end).
   - Ensure test cases cover:
     - Happy paths
     - Edge cases
     - Error scenarios
2. **Commit the Spec**
   - After planning, commit the spec file with the message:
     ```
     Plan: {story-key} - {brief-description}
     ```
     Example:
     ```
     Plan: 1-1-csv-mapping - Define field mapping requirements
     ```
